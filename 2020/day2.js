...
input.forEach(line => {
    ...
    const policyMin = strings[0].split("-")[0]
    const policyMax = strings[0].split("-")[1]
    const policyLetter = strings[1].split(":")[0]
    const password = strings[2]
    const chars = password.split("")
    let foundChars = 0
    let indexFoundChars = 0
    chars.forEach(char =>  {
        if (char === policyLetter) {
            foundChars++
        }
    })
    if (foundChars >= policyMin && foundChars <= policyMax) {
        valids++
    }
    if (chars[policyMin - 1] === policyLetter) {
        indexFoundChars++
    }
    if (chars[policyMax - 1] === policyLetter) {
        indexFoundChars++
    }
    if (indexFoundChars === 1) {
        indexValids++
    } 
})

console.log(valids)
console.log(indexValids)